...
public class DatabaseConnector implements PatientManager, StudyManager {
	...
	/*
	 * (non-Javadoc)
	 * @see entitiesManagers.StudyManager#getPatientStudies(entities.Patient) Para
	 * crear una statement a la db: Statement st = connection.createStatement();
	 * Para ejecutar un update (cualquier cosa que modifique la db):
	 * st.executeUpdate(<<SQL DE UN UPDATE O ALGO QUE NO DEVUELVA NADA>>); Para
	 * ejecutar un query (cosas que traigan resultados): ResultSet rs =
	 * st.executeQuery(<<SQL QUERY>>); while (rs.next()){ //ALGO QUE HAGAS A CADA
	 * LÍNEA DEL RESULTADO //POR EJEMPLO
	 * System.out.println("nombre="+rs.getObject("nombre")); //Hay getInt() y
	 * varios más //DONDE nombre ES EL NOMBRE DEL CAMPO } rs.close(); Siempre al
	 * final hay que cerrar el statement: st.close();
	 */
